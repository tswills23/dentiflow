...
## Recall LLM Reply Path

When a patient replies to a recall sequence and the inbound is NOT a critical intent (opt_out, urgent, wrong_number, slot_selection at S4), the reply may be generated by Claude Sonnet 4.5 instead of the keyword-based template. See [docs/eng/runbook-recall-llm.md](./docs/eng/runbook-recall-llm.md) for incident response.

**Hard rules (cannot be relaxed):**
- Critical intents NEVER reach Claude — keyword pre-filter in `classifyCriticalIntent()` is final
- Booking links, slot strings, opt-out confirmations, emergency handoffs always come from templates
- State transitions ALWAYS come from `getTransition()`. Claude's `next_state` is advisory only
- Three independent kill switches: `practices.recall_llm_enabled` (DB, fastest), `RECALL_LLM_FORCE_OFF` (env, in-memory bypass), `RECALL_LLM_ENABLED` (env, global)
- Hourly cap of 50 LLM replies per practice. Auto-fallback when reached
- Pinned model + temperature 0 — same input produces same output
- Monitor cron auto-disables LLM on first `validator_blocked` row

**Fastest rollback:** `UPDATE practices SET recall_llm_enabled = false WHERE id = '<id>'` — instant, no restart.

**Pre-deploy:** `npx tsx scripts/test-recall-replies.ts` — must pass 100% intent gate.
...
- `docs/` — see `docs/README.md` for the map (`eng/`, `gtm/`, `system/`, `reference/`)
- `scripts/` — live tools only; dated one-offs live in `scripts/_archive/`

## What does NOT live in this repo

Reorganized 2026-08-08. Do not re-add these here:

| Content | Where it went |
|---|---|
| Patient CSVs, Twilio exports, CallRail recordings (PHI) | `C:\Users\tswil\Dentiflow-PHI\` — never in a code checkout |
| Non-dental vertical research (vet, med spa, salon, gym, home services) | `C:\Users\tswil\vertical-research\` |
| GHL funnel HTML, paste-blocks, ad creative | `C:\Users\tswil\dentiflow-funnel\` |
| Scratch HTML, failed probes, type-test files | `C:\Users\tswil\Dentiflow-Archive\` |
